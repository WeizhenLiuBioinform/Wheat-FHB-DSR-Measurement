...
class PConv(nn.Module):
    def __init__(self, dim=int, n_div=int, forward="split_cat", kernel_sizes=[3, 5, 7]):
        super().__init__()
        if not isinstance(kernel_sizes, list):
            raise TypeError(f"Expected kernel_sizes to be a list, but got {type(kernel_sizes)}")
        ...
        self.extra_convs = nn.ModuleList([
            nn.Conv2d(self.dim_conv, self.dim_conv, ks, stride=1, padding=(ks - 1) // 2, bias=False)
            for ks in kernel_sizes[1:]
        ])

        # 权重计算模块
        self.weight_calc = nn.Conv2d(self.dim_conv * len(kernel_sizes), len(kernel_sizes), kernel_size=1)

    def forward(self, x):
        if self.forward_mode == "split_cat":
            x1, x2 = torch.split(x, [self.dim_conv, self.dim_untouched], dim=1)
            conv_outputs = [self.conv(x1)]  # 原始卷积
            for conv in self.extra_convs:
                conv_outputs.append(conv(x1))  # 额外的多尺度卷积
            conv_output = torch.cat(conv_outputs, dim=1)

            # 权重计算
            weights = torch.sigmoid(self.weight_calc(conv_output))
            weighted_sum = torch.sum(torch.stack([weights[:, i:i+1, :, :] * conv_outputs[i] for i in range(len(conv_outputs))]), dim=0)

            x = torch.cat((weighted_sum, x2), 1)
        elif self.forward_mode == "slicing":
            original_x = x[:, :self.dim_conv, :, :]
            conv_outputs = [self.conv(original_x)]
            for conv in self.extra_convs:
                conv_outputs.append(conv(original_x))
            conv_output = torch.cat(conv_outputs, dim=1)
...
class MFEBlock(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.cv1 = PConv(c1, 2, "split_cat", [3, 5, 7])
...
class MFEBlock(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = MSConv(c1, 2, "split_cat", [3, 5, 7])
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c2, 1, 1, g=g)
        self.add = shortcut
    def forward(self, x):
...
class MFE(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(MFEBlock(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
...
class Block(nn.Module):
    def __init__(self, c4):
        super().__init__()
        self.conv1 = Conv(c4, c4 // 2, 3, 1)
        self.conv2 = Conv(c4 // 2, c4 // 2, 3, 1)
        self.fusion = nn.Conv2d(c4, c4, 1, 1, bias=False)
...
#         self.cv2 = Conv(c3 // 2, c4, 1, 1)
#         self.blocks = nn.ModuleList([Block(c4) for _ in range(2)])  # 两个 Block
#         self.fusion = nn.Conv2d(c3 + 2 * c4, c2, 1, 1, bias=False)  # 修正输入通道数

#     def forward(self, x):
#         y1 = self.cv1(x)
#         y2_1, y2_2 = y1.chunk(2, dim=1)  # 使用 chunk 进行分割
#         y3 = self.cv2(y2_2)
#         y4 = y3

#         for block in self.blocks:
#             y4 = block(y4)
#             y3 = torch.cat([y3, y4], dim=1)

#         y = torch.cat([y2_1, y3], dim=1)
#         return self.fusion(y)

#     def forward_split(self, x):
#         y1 = self.cv1(x)
#         y2_1, y2_2 = y1.split(self.c, dim=1)  # 使用 split 进行分割
#         y3 = self.cv2(y2_2)
#         y4 = y3
...
#         # Second branch convolutions
#         self.conv2_3x3 = Conv(self.c_split, c4 // 4, 3, 1)
#         self.conv2_1x1 = Conv(c4 // 4, c4 // 4, 1, 1)

#         # Third branch convolutions
#         self.conv3_5x5 = Conv(self.c_split, c4 // 4, 5, 1)
#         self.conv3_1x1 = Conv(c4 // 4, c4 // 4, 1, 1)

#         # Fourth branch convolutions
#         self.conv4_7x7 = Conv(self.c_split, self.c4_split, 7, 1)
#         self.conv4_5x5 = Conv(self.c4_split, self.c4_split, 5, 1)
#         self.conv4_3x3 = Conv(self.c4_split, self.c4_split, 3, 1)
#         self.conv4_1x1 = Conv(self.c4_split, self.c4_split, 1, 1)

#         self.conv_concat = Conv(c3 + (2 * c4), c2, 1, 1)
...
#         # Second branch
#         branch2 = self.conv2_3x3(y_split[1])
#         concat2 = [branch2]
#         branch2 = self.conv2_1x1(branch2)
#         concat2.append(branch2)
        
#         # Third branch
#         branch3 = self.conv3_5x5(y_split[2])
#         concat3 = [branch3]
#         branch3 = self.conv3_1x1(branch3)
#         concat3.append(branch3)

#         # Fourth branch
#         branch4 = self.conv4_7x7(y_split[3])
#         concat4 = [branch4]
#         branch4 = self.conv4_5x5(branch4)
#         concat4.append(branch4)
#         branch4 = self.conv4_3x3(branch4)
#         concat4.append(branch4)
#         branch4 = self.conv4_1x1(branch4)
#         concat4.append(branch4)

#         # Concatenate all branches
#         concat = torch.cat([branch1, y_split[1], y_split[2], y_split[3]] + concat2 + concat3 + concat4, dim=1)
...
#     def forward_split(self, x):
#         y = self.conv1(x)
#         y_split = y.split(self.c_split, 1)
