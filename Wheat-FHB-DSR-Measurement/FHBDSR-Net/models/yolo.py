...
from models.common import C3, ELAN1, C2f, C2fAttn, C3Ghost, GhostBottleneck, GhostConv, RepC3, MFE
from models.yolov10 import PSA
...
from models.yolov10_modules import PSA, SCDown,SA_GELAN,SA_GELAN2,SA_GELAN3,Diffusion_PSA,Diffusion_PSA2,Diffusion_PSA3,Diffusion_PSA4,Diffusion_PSA6,Diffusion_PSA7,CrossDomain_PSA,SA_GELAN4,SA_GELAN5_Optimized,SA_GELAN_FFT,SA_GELAN_FFT2,SA_GELAN_FFT3,SA_GELAN_FFT4
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            RepNCSPELAN4, SPPELAN,C3,RepC3,C2fAttn,DWConv,GhostBottleneck,GhostConv,C2f, ELAN1,PSA, SCDown,PSA,SA_GELAN,SA_GELAN2,SA_GELAN3,Diffusion_PSA,Diffusion_PSA2,Diffusion_PSA3,Diffusion_PSA4,Diffusion_PSA6,Diffusion_PSA7,CrossDomain_PSA,SA_GELAN4,SA_GELAN5_Optimized,SA_GELAN_FFT,SA_GELAN_FFT2,SA_GELAN_FFT3, SA_GELAN_FFT4}:
            ...
            if m in (RepNCSPELAN4,):
                ...
                args[3] = max(round(args[3] * gd), 1) if n > 1 else n #https://blog.csdn.net/java1314777/article/details/136350163


            if m is C2fAttn:
                args[1] = make_divisible( args[1] * gw, 8)  # embed channels
                args[2] = int(
                    max(round(args[2]) * gw, 1) if args[2] > 1 else args[2]  
                )  # num heads

            args = [c1, c2, *args[1:]]
            
            if m in {BottleneckCSP, SPPCSPC,C2f,C3Ghost,RepC3,C2fAttn,C3}:
                args.insert(2, n)  # number of repeats
                n = 1

        elif m in [MFE, FasterNeXt]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
            if m in [MFE, FasterNeXt]:
                args.insert(2, n)  # number of repeats
                n = 1
        ...
        ###attention #####
        elif m in {DoubleAttention}:
            c2 = ch[f]
            args = [c2, *args]
 
        ###attention #####
        elif m in (SimAM, CBAM):
            c1, c2 = ch[f], args[0]
